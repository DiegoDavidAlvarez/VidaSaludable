    ...
    <div class="w-full bg-zinc-900 rounded-xl shadow-2xl overflow-hidden p-6 border border-zinc-800">
        <div class="flex justify-between items-center mb-6">
            ...
            <div class="space-x-2">
                ...
                <a href="{{ route('admin.purchase_detail.export-excel') }}"
                    ...
                    @foreach ($purchaseDetails as $detail)
                        <tr>
                            ...
                            <td class="px-4 py-4 text-sm text-zinc-300">
                                ...
                                <br>
                                <span class="text-xs text-zinc-400">Total: S/. {{ number_format($detail->purchase->total, 2) }}</span>
                            </td>
                            ...
                            <td class="px-4 py-4 text-sm text-zinc-300">{{ number_format($detail->unitary_cost, 2) }}</td>
                            <td class="px-4 py-4 text-sm text-right">
                                <!-- Botón Editar -->
                                <button
                                    @click="openModal({{ $detail->id }}, {{ $detail->purchase_id }}, {{ $detail->product_id }}, {{ $detail->quantity }}, {{ $detail->unitary_cost }})"
                                ...
                                <form id="delete-form-{{ $detail->id }}"
                                    action="{{ route('admin.purchase_detail.destroy', $detail->id) }}" method="POST"
                                    class="hidden">
                                    @csrf
                                    @method('DELETE')
                                </form>
        ...
        <div x-show="isOpen" x-cloak x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0"
            ...
            <div class="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                ...
                <div
                    ...
                    <form :action="'/admin/purchase_detail/' + currentId" method="POST">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="px-8 py-8">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div>
                                    ...
                                    <select x-model="currentPurchaseId" name="purchase_id"
                                        ...
                                        @foreach ($purchases as $purchase)
                                            <option value="{{ $purchase->id }}">{{ $purchase->date }} - S/. {{ number_format($purchase->total, 2) }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <!-- Campo Producto -->
                                ...
                                <div>
                                    ...
                                    <input type="number" x-model="currentQuantity" name="quantity"
                                ...
                                <div>
                                    ...
                                    <input type="number" x-model="currentUnitaryCost" name="unitary_cost"
                                        pattern="^\d{1,8}(\.\d{0,2})?$"
                                        maxlength="11"
...
<script>
    ...
    function detailTable() {
        return {
            ...
            currentQuantity: 1,
            currentUnitaryCost: 0.00,

            openModal(id, purchase_id, product_id, quantity, unitary_cost) {
                this.currentId = id;
                this.currentPurchaseId = purchase_id;
                this.currentProductId = product_id;
                this.currentQuantity = quantity;
                this.currentUnitaryCost = unitary_cost;
                this.isOpen = true;
