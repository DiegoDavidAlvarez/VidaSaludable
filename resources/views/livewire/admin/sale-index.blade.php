...
<div class="w-full py-8 px-4 sm:px-6 lg:px-8 space-y-8">
    ...
    <div class="w-full bg-zinc-900 rounded-xl shadow-2xl overflow-hidden p-6 border border-zinc-800">
        ...
        <form id="sales-form" action="{{ route('admin.sale.store') }}" method="POST" class="space-y-6" data-flux-component="form">
            @csrf
...
<script>
    $(document).ready(function () {
        ...
        $('#consultar-cliente').on('click', function () {
            ...
            $.ajax({
                url: '{{ route('admin.sale.get-customer') }}',
                method: 'POST',
        ...
        addProductButton.on('click', function () {
            ...
            $.ajax({
                url: '{{ route('admin.sale.get-product') }}',
                method: 'POST',
                data: JSON.stringify({ bar_code: barcode }),
                contentType: 'application/json',
                headers: { 'X-CSRF-TOKEN': '{{ csrf_token() }}' },
                success: function (data) {
                    if (data.product) {
                        const existingProductIndex = products.findIndex(p => p.bar_code === data.product.bar_code);
                        if (existingProductIndex !== -1) {
                            products[existingProductIndex].quantity += 1;
                        } else {
                            products.push({
                                bar_code: data.product.bar_code,
                                quantity: 1,
                                name: data.product.name,
                                unitary_price: parseFloat(data.product.sale_price),
        ...
        function updateProductList() {
            ...
            products.forEach((product, index) => {
                ...
                const subtotal = product.unitary_price * product.quantity;
                total += subtotal;
                const row = `
                    <tr class="hover:bg-zinc-700/50 transition-colors">
                        <td class="p-3">${product.name}</td>
                        <td class="p-3">S/ ${product.unitary_price.toFixed(2)}</td>
                        <td class="p-3">
...
7h16" />
                ...
                const productInputs = `
                    <input type="hidden" name="products[${index}][bar_code]" value="${product.bar_code}">
